...
function clean_run() {
    COUNT_FAILURES=0 AVERAGE=0 RESTART=0 PATCH=0 TIME_RUNS=0 "${BASH_SOURCE[0]}" "$1"
}
...
function restart() {
    if [[ "$RESTART" == 1 ]]; then
        echo ", (re)starting"
        # Create a placeholder jobout, while the real job is in the queue
