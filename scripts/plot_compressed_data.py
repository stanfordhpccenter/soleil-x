    ...
    if timestep_group != 'timestep_0':
      ...
      for scalar_feild_name in scalar_data_to_plot: 
        ...
        plt.pcolormesh(np.array(z_points),
                       np.array(y_points),
                       h5_file[timestep_group]['mid_x_slice_{}'.format(scalar_feild_name)],
                       vmin =  stuff_for_plots['mid_x_slice_{}'.format(feild_name)]['min'],
                       vmax =  stuff_for_plots['mid_x_slice_{}'.format(feild_name)]['max'])
        ## Add where duct would be
        #rect = Rectangle((0.0, y_duct_start), Lz, duct_width, edgecolor='k', facecolor='none', linestyle='--')
        #ax = plt.gca()
        #ax.add_patch(rect)
        ...
        cax = divider.append_axes("right", size="2%", pad=0.05)
        plt.colorbar(cax=cax)
        plt.savefig(os.path.join(output_dir,'{}_mid_x_slice_{:0>4d}.png'.format(scalar_feild_name,timestep)), bbox_inches='tight')
        plt.close()
      ...
      for scalar_feild_name in scalar_data_to_plot: 
        ...
        plt.pcolormesh(y_points,
                       ...
                       vmin =  stuff_for_plots['measurement_z_slice_{}'.format(feild_name)]['min'],
                       vmax =  stuff_for_plots['measurement_z_slice_{}'.format(feild_name)]['max'])
        ...
        cax = divider.append_axes("right", size="2%", pad=0.05)
        plt.colorbar(cax=cax)
        plt.savefig(os.path.join(output_dir,'measurement_plane_{}_slice_{:04d}.png'.format(scalar_feild_name, timestep)), dpi=300, bbox_inches='tight')
        plt.close()
      ...
      for scalar_feild_name in scalar_data_to_plot: 
        ...
        plt.xlabel(r'$x \ [m]$', fontsize = 20)
        plt.ylabel('{} {}'.format(title_labels[scalar_feild_name], unit_labels[scalar_feild_name]), fontsize = 20)
        plt.ylim([stuff_for_plots['{}_spanwise_average'.format(feild_name)]['min'], stuff_for_plots['{}_spanwise_average'.format(feild_name)]['max']])
        plt.title('{} Spanwise Average'.format(title_labels[scalar_feild_name]), fontsize = 20)
        plt.legend(loc='upper left')
        plt.savefig(os.path.join(output_dir,'{}_spanwise_average_{:0>4d}.png'.format(scalar_feild_name,timestep)), bbox_inches='tight')
        plt.close()

      # Temperature on Measurement Line
      for scalar_feild_name in scalar_data_to_plot: 
        plt.figure()
        ...
        plt.plot(time_averaged_data['measurement_line_{}'.format(scalar_feild_name)],
                 ...
                 '--k',
                 label='time average')
        plt.xlim([stuff_for_plots['measurement_z_slice_{}'.format(feild_name)]['min'],stuff_for_plots['measurement_z_slice_{}'.format(feild_name)]['max']])
        plt.xlabel('{} {}'.format(title_labels[scalar_feild_name], unit_labels[scalar_feild_name]), fontsize = 20)
        plt.ylabel(r'$y$ [m]', fontsize = 20)
        plt.title('{} on Measurement Line'.format(title_labels[scalar_feild_name]), fontsize = 20)
        plt.savefig(os.path.join(output_dir,'measurement_line_{}_{:04d}.png'.format(scalar_feild_name, timestep)), dpi=300, bbox_inches='tight')
        plt.close()
