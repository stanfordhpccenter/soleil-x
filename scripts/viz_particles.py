...
parser.add_argument('hdf_file', nargs='+', help='simulation results to visualize')
args = parser.parse_args()

hdf_out_basename = 'out_particles_'
num_particles = numpy.empty(len(args.hdf_file))
for (f, i) in zip(args.hdf_file, itertools.count()):
    ...
    num_particles[i] = int(sum(valids))
    hdf_out.close()
    hdf_in.close()
...
with open('out_particles.xmf', 'w') as xmf_out:
    ...
    for i in range(len(args.hdf_file)):
        xmf_out.write(XMF_BODY
                      ...
                      .replace('@NUM_PARTICLES', str(int(num_particles[i])))
