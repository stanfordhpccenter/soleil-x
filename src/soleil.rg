...
task make_interior_partition_x_hi(faces : region(ispace(int3d), face), x_tiles : ispace(int3d), Nx : int32, Ny : int32, Nz : int32, ntx : int32, nty : int32, ntz : int32)
  ...
  for tile in x_tiles do
    ...
    var lo = int3d({x = ((tile.x*Nx)/ntx), y = ((tile.y*Ny)/nty), z = ((tile.z*Nz)/ntz)})
    var hi = int3d({x = ((((tile.x+1)*Nx)/ntx)+val), y = ((((tile.y+1)*Ny)/nty)-1), z = ((((tile.z+1)*Nz)/ntz)-1)})
    if (hi.x>=(Nx+1)) then
      lo.x = int64(1)
      hi.x = int64(0)
    else
    ...
    regentlib.c.legion_domain_point_coloring_color_domain(coloring, regentlib.c.legion_domain_point_t(tile), regentlib.c.legion_domain_t(rect))
  end
...
task make_interior_partition_x_lo(faces : region(ispace(int3d), face), x_tiles : ispace(int3d), Nx : int32, Ny : int32, Nz : int32, ntx : int32, nty : int32, ntz : int32)
  var coloring = regentlib.c.legion_domain_point_coloring_create()
  for tile in x_tiles do
    var val = 1
    if (tile.x==1) then
    ...
    var lo = int3d({x = ((((tile.x-1)*Nx)/ntx)+val), y = ((tile.y*Ny)/nty), z = ((tile.z*Nz)/ntz)})
    var hi = int3d({x = ((tile.x*Nx)/ntx), y = ((((tile.y+1)*Ny)/nty)-1), z = ((((tile.z+1)*Nz)/ntz)-1)})
    if (lo.x<0) then
      lo.x = int64(1)
      hi.x = int64(0)
    else
...
task make_interior_partition_y_hi(faces : region(ispace(int3d), face), y_tiles : ispace(int3d), Nx : int32, Ny : int32, Nz : int32, ntx : int32, nty : int32, ntz : int32)
  var coloring = regentlib.c.legion_domain_point_coloring_create()
  for tile in y_tiles do
    ...
    var lo = int3d({x = ((tile.x*Nx)/ntx), y = ((tile.y*Ny)/nty), z = ((tile.z*Nz)/ntz)})
    var hi = int3d({x = ((((tile.x+1)*Nx)/ntx)-1), y = ((((tile.y+1)*Ny)/nty)+val), z = ((((tile.z+1)*Nz)/ntz)-1)})
    if (hi.y>=(Ny+1)) then
      lo.y = int64(1)
      hi.y = int64(0)
    else
...
task make_interior_partition_y_lo(faces : region(ispace(int3d), face), y_tiles : ispace(int3d), Nx : int32, Ny : int32, Nz : int32, ntx : int32, nty : int32, ntz : int32)
  var coloring = regentlib.c.legion_domain_point_coloring_create()
  for tile in y_tiles do
    var val = 1
    if (tile.y==1) then
    ...
    var lo = int3d({x = ((tile.x*Nx)/ntx), y = ((((tile.y-1)*Ny)/nty)+val), z = ((tile.z*Nz)/ntz)})
    var hi = int3d({x = ((((tile.x+1)*Nx)/ntx)-1), y = ((tile.y*Ny)/nty), z = ((((tile.z+1)*Nz)/ntz)-1)})
    if (lo.y<0) then
      lo.y = int64(1)
      hi.y = int64(0)
    else
...
task make_interior_partition_z_hi(faces : region(ispace(int3d), face), z_tiles : ispace(int3d), Nx : int32, Ny : int32, Nz : int32, ntx : int32, nty : int32, ntz : int32)
  var coloring = regentlib.c.legion_domain_point_coloring_create()
  for tile in z_tiles do
    ...
    var lo = int3d({x = ((tile.x*Nx)/ntx), y = ((tile.y*Ny)/nty), z = ((tile.z*Nz)/ntz)})
    var hi = int3d({x = ((((tile.x+1)*Nx)/ntx)-1), y = ((((tile.y+1)*Ny)/nty)-1), z = ((((tile.z+1)*Nz)/ntz)+val)})
    if (hi.z>=(Nz+1)) then
      lo.z = int64(1)
      hi.z = int64(0)
    else
...
task make_interior_partition_z_lo(faces : region(ispace(int3d), face), z_tiles : ispace(int3d), Nx : int32, Ny : int32, Nz : int32, ntx : int32, nty : int32, ntz : int32)
  var coloring = regentlib.c.legion_domain_point_coloring_create()
  for tile in z_tiles do
    var val = 1
    if (tile.z==1) then
    ...
    var lo = int3d({x = ((tile.x*Nx)/ntx), y = ((tile.y*Ny)/nty), z = ((((tile.z-1)*Nz)/ntz)+val)})
    var hi = int3d({x = ((((tile.x+1)*Nx)/ntx)-1), y = ((((tile.y+1)*Ny)/nty)-1), z = ((tile.z*Nz)/ntz)})
    if (lo.z<0) then
      lo.z = int64(1)
      hi.z = int64(0)
    else
    ...
    for k : int64 = limits.lo.z, (limits.hi.z+1) do
      ...
      for m : int32 = 0, 14 do
        if (angles[m].xi<0) then
          ...
          reflect += (((((1-epsw)/3.1415926535898)*angles[m].w)*C.fabs(angles[m].xi))*face_value)
        else
        end
      end
      for m : int32 = 0, 14 do
        if (angles[m].xi>0) then
          var value = ((((epsw*5.67e-08)*C.pow(Tw, 4))/3.1415926535898)+reflect)
          if ((angles[m].eta>0) and (angles[m].mu>0)) then
    ...
    for k : int64 = limits.lo.z, (limits.hi.z+1) do
      ...
      for m : int32 = 0, 14 do
        if (angles[m].eta<0) then
          ...
          reflect += (((((1-epsw)/3.1415926535898)*angles[m].w)*C.fabs(angles[m].eta))*face_value)
        else
        end
      end
      for m : int32 = 0, 14 do
    ...
    for j : int64 = limits.lo.y, (limits.hi.y+1) do
      ...
      for m : int32 = 0, 14 do
        if (angles[m].mu<0) then
          ...
          reflect += (((((1-epsw)/3.1415926535898)*angles[m].w)*C.fabs(angles[m].mu))*face_value)
        else
        end
      end
      for m : int32 = 0, 14 do
...
do
  ...
  var dindx = int64(1)
  var startx = limits.lo.x
  var endx = (limits.hi.x+1)
  var dindy = int64(1)
  var starty = limits.lo.y
  var endy = (limits.hi.y+1)
  var dindz = int64(1)
  var startz = limits.lo.z
  var endz = (limits.hi.z+1)
  if (xi<0) then
    dindx = int64((-1))
    startx = limits.hi.x
    endx = (limits.lo.x-1)
      ...
      for k : int64 = startz, endz, dindz do
        for j : int64 = starty, endy, dindy do
          for i : int64 = startx, endx, dindx do
            var indx = (i-min(dindx, 0))
            var indy = (j-min(dindy, 0))
            var indz = (k-min(dindz, 0))
            var ghost_x_limits = ghost_x_faces.bounds
            var upwind_x_value = 0
            ...
            points[{i, j, k}].I_1[m] = (((((points[{i, j, k}].S*dV)+(((C.fabs(angles[m].xi)*dAx)*upwind_x_value)/0.5))+(((C.fabs(angles[m].eta)*dAy)*upwind_y_value)/0.5))+(((C.fabs(angles[m].mu)*dAz)*upwind_z_value)/0.5))/((((points[{i, j, k}].sigma*dV)+((C.fabs(angles[m].xi)*dAx)/0.5))+((C.fabs(angles[m].eta)*dAy)/0.5))+((C.fabs(angles[m].mu)*dAz)/0.5)))
      ...
      for k : int64 = startz, endz, dindz do
        for j : int64 = starty, endy, dindy do
          for i : int64 = startx, endx, dindx do
            ...
            points[{i, j, k}].I_2[m] = (((((points[{i, j, k}].S*dV)+(((C.fabs(angles[m].xi)*dAx)*upwind_x_value)/0.5))+(((C.fabs(angles[m].eta)*dAy)*upwind_y_value)/0.5))+(((C.fabs(angles[m].mu)*dAz)*upwind_z_value)/0.5))/((((points[{i, j, k}].sigma*dV)+((C.fabs(angles[m].xi)*dAx)/0.5))+((C.fabs(angles[m].eta)*dAy)/0.5))+((C.fabs(angles[m].mu)*dAz)/0.5)))
      ...
      for k : int64 = startz, endz, dindz do
        for j : int64 = starty, endy, dindy do
          for i : int64 = startx, endx, dindx do
            ...
            points[{i, j, k}].I_3[m] = (((((points[{i, j, k}].S*dV)+(((C.fabs(angles[m].xi)*dAx)*upwind_x_value)/0.5))+(((C.fabs(angles[m].eta)*dAy)*upwind_y_value)/0.5))+(((C.fabs(angles[m].mu)*dAz)*upwind_z_value)/0.5))/((((points[{i, j, k}].sigma*dV)+((C.fabs(angles[m].xi)*dAx)/0.5))+((C.fabs(angles[m].eta)*dAy)/0.5))+((C.fabs(angles[m].mu)*dAz)/0.5)))
      ...
      for k : int64 = startz, endz, dindz do
        for j : int64 = starty, endy, dindy do
          for i : int64 = startx, endx, dindx do
            ...
            points[{i, j, k}].I_4[m] = (((((points[{i, j, k}].S*dV)+(((C.fabs(angles[m].xi)*dAx)*upwind_x_value)/0.5))+(((C.fabs(angles[m].eta)*dAy)*upwind_y_value)/0.5))+(((C.fabs(angles[m].mu)*dAz)*upwind_z_value)/0.5))/((((points[{i, j, k}].sigma*dV)+((C.fabs(angles[m].xi)*dAx)/0.5))+((C.fabs(angles[m].eta)*dAy)/0.5))+((C.fabs(angles[m].mu)*dAz)/0.5)))
      ...
      for k : int64 = startz, endz, dindz do
        for j : int64 = starty, endy, dindy do
          for i : int64 = startx, endx, dindx do
            ...
            points[{i, j, k}].I_5[m] = (((((points[{i, j, k}].S*dV)+(((C.fabs(angles[m].xi)*dAx)*upwind_x_value)/0.5))+(((C.fabs(angles[m].eta)*dAy)*upwind_y_value)/0.5))+(((C.fabs(angles[m].mu)*dAz)*upwind_z_value)/0.5))/((((points[{i, j, k}].sigma*dV)+((C.fabs(angles[m].xi)*dAx)/0.5))+((C.fabs(angles[m].eta)*dAy)/0.5))+((C.fabs(angles[m].mu)*dAz)/0.5)))
      ...
      for k : int64 = startz, endz, dindz do
        for j : int64 = starty, endy, dindy do
          for i : int64 = startx, endx, dindx do
            ...
            points[{i, j, k}].I_6[m] = (((((points[{i, j, k}].S*dV)+(((C.fabs(angles[m].xi)*dAx)*upwind_x_value)/0.5))+(((C.fabs(angles[m].eta)*dAy)*upwind_y_value)/0.5))+(((C.fabs(angles[m].mu)*dAz)*upwind_z_value)/0.5))/((((points[{i, j, k}].sigma*dV)+((C.fabs(angles[m].xi)*dAx)/0.5))+((C.fabs(angles[m].eta)*dAy)/0.5))+((C.fabs(angles[m].mu)*dAz)/0.5)))
      ...
      for k : int64 = startz, endz, dindz do
        for j : int64 = starty, endy, dindy do
          for i : int64 = startx, endx, dindx do
            ...
            points[{i, j, k}].I_7[m] = (((((points[{i, j, k}].S*dV)+(((C.fabs(angles[m].xi)*dAx)*upwind_x_value)/0.5))+(((C.fabs(angles[m].eta)*dAy)*upwind_y_value)/0.5))+(((C.fabs(angles[m].mu)*dAz)*upwind_z_value)/0.5))/((((points[{i, j, k}].sigma*dV)+((C.fabs(angles[m].xi)*dAx)/0.5))+((C.fabs(angles[m].eta)*dAy)/0.5))+((C.fabs(angles[m].mu)*dAz)/0.5)))
      ...
      for k : int64 = startz, endz, dindz do
        for j : int64 = starty, endy, dindy do
          for i : int64 = startx, endx, dindx do
            ...
            points[{i, j, k}].I_8[m] = (((((points[{i, j, k}].S*dV)+(((C.fabs(angles[m].xi)*dAx)*upwind_x_value)/0.5))+(((C.fabs(angles[m].eta)*dAy)*upwind_y_value)/0.5))+(((C.fabs(angles[m].mu)*dAz)*upwind_z_value)/0.5))/((((points[{i, j, k}].sigma*dV)+((C.fabs(angles[m].xi)*dAx)/0.5))+((C.fabs(angles[m].eta)*dAy)/0.5))+((C.fabs(angles[m].mu)*dAz)/0.5)))
...
do
  ...
  for p in points do
    for m : int32 = 0, 14 do
      if (p.I_1[m]>0) then
        res += (((1/(((Nx*Ny)*Nz)*14))*C.pow((p.I_1[m]-p.Iiter_1[m]), 2))/C.pow(p.I_1[m], 2))
      else
      end
      if (p.I_2[m]>0) then
        res += (((1/(((Nx*Ny)*Nz)*14))*C.pow((p.I_2[m]-p.Iiter_2[m]), 2))/C.pow(p.I_2[m], 2))
      else
      end
      if (p.I_3[m]>0) then
        res += (((1/(((Nx*Ny)*Nz)*14))*C.pow((p.I_3[m]-p.Iiter_3[m]), 2))/C.pow(p.I_3[m], 2))
      else
      end
      if (p.I_4[m]>0) then
        res += (((1/(((Nx*Ny)*Nz)*14))*C.pow((p.I_4[m]-p.Iiter_4[m]), 2))/C.pow(p.I_4[m], 2))
      else
      end
      if (p.I_5[m]>0) then
        res += (((1/(((Nx*Ny)*Nz)*14))*C.pow((p.I_5[m]-p.Iiter_5[m]), 2))/C.pow(p.I_5[m], 2))
      else
      end
      if (p.I_6[m]>0) then
        res += (((1/(((Nx*Ny)*Nz)*14))*C.pow((p.I_6[m]-p.Iiter_6[m]), 2))/C.pow(p.I_6[m], 2))
      else
      end
      if (p.I_7[m]>0) then
        res += (((1/(((Nx*Ny)*Nz)*14))*C.pow((p.I_7[m]-p.Iiter_7[m]), 2))/C.pow(p.I_7[m], 2))
      else
      end
      if (p.I_8[m]>0) then
        res += (((1/(((Nx*Ny)*Nz)*14))*C.pow((p.I_8[m]-p.Iiter_8[m]), 2))/C.pow(p.I_8[m], 2))
      else
      end
    end
  end
...
do
  ...
  for p in particles do
    if particles[p].__valid then
      ...
      particles[p].deltaTemperatureTerm = (((double(3.1415926535898)*C.pow(particles[p].diameter, 2.0))*Particles_convectiveCoeff)*(flowTemperature-particles[p].temperature))
      ...
      particles[p].temperature_t += (particles[p].deltaTemperatureTerm/((((double(3.1415926535898)*C.pow(particles[p].diameter, 3.0))/6.0)*particles[p].density)*Particles_heatCapacity))
    else
    end
  end
end
...
do
  __demand(__openmp)
  for c in Radiation do
    Radiation[c].sigma = (((Radiation[c].acc_d2*double(3.1415926535898))*(Radiation_qa+Radiation_qs))/(4.0*Radiation_cellVolume))
...
do
  ...
  for p in particles do
    if particles[p].__valid then
      ...
      var alpha = ((((double(3.1415926535898)*Radiation_qa)*C.pow(particles[p].diameter, 2.0))*(Radiation[Fluid[particles[p].cell].to_Radiation].G-((4.0*double(5.67e-08))*t4)))/4.0)
      particles[p].temperature_t += (alpha/((((double(3.1415926535898)*C.pow(particles[p].diameter, 3.0))/6.0)*particles[p].density)*Particles_heatCapacity))
    else
    end
  end
end
...
do
  ...
  for p in particles do
    if particles[p].__valid then
      var tmp = vs_div_double_3(vs_mul_double_3(particles[p].deltaVelocityOverRelaxationTime, (-(((double(3.1415926535898)*C.pow(particles[p].diameter, 3.0))/6.0)*particles[p].density))), Grid_cellVolume)
...
task work(config : Config)
  ...
  __parallelize_with Fluid_primPart, particles_primPart, Radiation_primPart, primColors, (image(Fluid, particles_primPart, particles.cell)<=Fluid_primPart) do
    ...
    var grid_x = ispace(int3d, int3d({x = (Radiation_xNum+1), y = Radiation_yNum, z = Radiation_zNum}))
    var grid_y = ispace(int3d, int3d({x = Radiation_xNum, y = (Radiation_yNum+1), z = Radiation_zNum}))
    var grid_z = ispace(int3d, int3d({x = Radiation_xNum, y = Radiation_yNum, z = (Radiation_zNum+1)}))
    ...
    var angle_indices = ispace(int1d, int1d(14))
    var angles = region(angle_indices, angle)
    var x_tiles = ispace(int3d, int3d({x = (ntx+1), y = nty, z = ntz}))
    var y_tiles = ispace(int3d, int3d({x = ntx, y = (nty+1), z = ntz}))
    var z_tiles = ispace(int3d, int3d({x = ntx, y = nty, z = (ntz+1)}))
    var p_x_faces_1 = make_interior_partition_x_lo(x_faces_1, x_tiles, Radiation_xNum, Radiation_yNum, Radiation_zNum, ntx, nty, ntz)
    var p_y_faces_1 = make_interior_partition_y_lo(y_faces_1, y_tiles, Radiation_xNum, Radiation_yNum, Radiation_zNum, ntx, nty, ntz)
    var p_z_faces_1 = make_interior_partition_z_lo(z_faces_1, z_tiles, Radiation_xNum, Radiation_yNum, Radiation_zNum, ntx, nty, ntz)
    var p_x_faces_2 = make_interior_partition_x_lo(x_faces_2, x_tiles, Radiation_xNum, Radiation_yNum, Radiation_zNum, ntx, nty, ntz)
    var p_y_faces_2 = make_interior_partition_y_lo(y_faces_2, y_tiles, Radiation_xNum, Radiation_yNum, Radiation_zNum, ntx, nty, ntz)
    var p_z_faces_2 = make_interior_partition_z_hi(z_faces_2, z_tiles, Radiation_xNum, Radiation_yNum, Radiation_zNum, ntx, nty, ntz)
    var p_x_faces_3 = make_interior_partition_x_lo(x_faces_3, x_tiles, Radiation_xNum, Radiation_yNum, Radiation_zNum, ntx, nty, ntz)
    var p_y_faces_3 = make_interior_partition_y_hi(y_faces_3, y_tiles, Radiation_xNum, Radiation_yNum, Radiation_zNum, ntx, nty, ntz)
    var p_z_faces_3 = make_interior_partition_z_lo(z_faces_3, z_tiles, Radiation_xNum, Radiation_yNum, Radiation_zNum, ntx, nty, ntz)
    var p_x_faces_4 = make_interior_partition_x_lo(x_faces_4, x_tiles, Radiation_xNum, Radiation_yNum, Radiation_zNum, ntx, nty, ntz)
    var p_y_faces_4 = make_interior_partition_y_hi(y_faces_4, y_tiles, Radiation_xNum, Radiation_yNum, Radiation_zNum, ntx, nty, ntz)
    var p_z_faces_4 = make_interior_partition_z_hi(z_faces_4, z_tiles, Radiation_xNum, Radiation_yNum, Radiation_zNum, ntx, nty, ntz)
    var p_x_faces_5 = make_interior_partition_x_hi(x_faces_5, x_tiles, Radiation_xNum, Radiation_yNum, Radiation_zNum, ntx, nty, ntz)
    var p_y_faces_5 = make_interior_partition_y_lo(y_faces_5, y_tiles, Radiation_xNum, Radiation_yNum, Radiation_zNum, ntx, nty, ntz)
    var p_z_faces_5 = make_interior_partition_z_lo(z_faces_5, z_tiles, Radiation_xNum, Radiation_yNum, Radiation_zNum, ntx, nty, ntz)
    var p_x_faces_6 = make_interior_partition_x_hi(x_faces_6, x_tiles, Radiation_xNum, Radiation_yNum, Radiation_zNum, ntx, nty, ntz)
    var p_y_faces_6 = make_interior_partition_y_lo(y_faces_6, y_tiles, Radiation_xNum, Radiation_yNum, Radiation_zNum, ntx, nty, ntz)
    var p_z_faces_6 = make_interior_partition_z_hi(z_faces_6, z_tiles, Radiation_xNum, Radiation_yNum, Radiation_zNum, ntx, nty, ntz)
    var p_x_faces_7 = make_interior_partition_x_hi(x_faces_7, x_tiles, Radiation_xNum, Radiation_yNum, Radiation_zNum, ntx, nty, ntz)
    var p_y_faces_7 = make_interior_partition_y_hi(y_faces_7, y_tiles, Radiation_xNum, Radiation_yNum, Radiation_zNum, ntx, nty, ntz)
    var p_z_faces_7 = make_interior_partition_z_lo(z_faces_7, z_tiles, Radiation_xNum, Radiation_yNum, Radiation_zNum, ntx, nty, ntz)
    var p_x_faces_8 = make_interior_partition_x_hi(x_faces_8, x_tiles, Radiation_xNum, Radiation_yNum, Radiation_zNum, ntx, nty, ntz)
    var p_y_faces_8 = make_interior_partition_y_hi(y_faces_8, y_tiles, Radiation_xNum, Radiation_yNum, Radiation_zNum, ntx, nty, ntz)
    var p_z_faces_8 = make_interior_partition_z_hi(z_faces_8, z_tiles, Radiation_xNum, Radiation_yNum, Radiation_zNum, ntx, nty, ntz)
