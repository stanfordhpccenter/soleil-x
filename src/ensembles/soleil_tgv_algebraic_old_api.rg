...
assert(json_dir)
local JSON = terralib.includec("json.h", {"-I", json_dir .. "/include/json-parser"})
...
terra particles_getBasePointer(pr : regentlib.c.legion_physical_region_t,fid : uint32,runtime : regentlib.c.legion_runtime_t) : &opaque
    var acc = regentlib.c.legion_physical_region_get_field_accessor_generic(pr, fid)
    ...
    var p = regentlib.c.legion_accessor_generic_raw_rect_ptr_1d(acc, rect, &subrect, &[&regentlib.c.legion_byte_offset_t](offsets)[0])
    regentlib.c.legion_accessor_generic_destroy(acc)
    return p
end
