...
local json_lib_dir = os.getenv("JSON_LIB_DIR")
assert(json_lib_dir)
...
terra parseConfig(filename : &int8) : Config
    var config : Config
    ...
    var res2 = C.fseek(f, [int64](0), 0)
    if res2 ~= 0 then
        C.printf("%s\n", "Cannot seek to start of config file")
        C.exit(1)
    end
    var buf = [&int8](C.malloc([uint64](len)))
    if buf == [&int8](nil) then
        C.printf("%s\n", "Malloc error while parsing config")
        C.exit(1)
    end
    var res3 = C.fread([&opaque](buf), [uint64](1), [uint64](len), f)
    if res3 < [uint64](len) then
    ...
    settings.settings = 1
    var root = JSON.json_parse_ex(&settings, buf, [uint64](len), [&int8](errMsg))
    if root == [&JSON._json_value](nil) then
        C.printf("JSON parsing error: %s\n", [&int8](errMsg))
        C.exit(1)
    end
    var totalParsed = 0
    if root.type ~= [uint32](1) then
        C.printf("%s for option %s\n", "Wrong type", "<root>")
        C.exit(1)
    end
        for i : uint32 = [uint32](0),root.u.object.length do
            var nodeName = root.u.object.values[i].name
            var nodeValue = root.u.object.values[i].value
            var parsed = false
            if C.strcmp(nodeName, "IO") == 0 then
                var totalParsedv1 = 0
                if nodeValue.type ~= [uint32](1) then
                    C.printf("%s for option %s\n", "Wrong type", nodeName)
                    C.exit(1)
                end
                    for iv1 : uint32 = [uint32](0),nodeValue.u.object.length do
                        var nodeNamev1 = nodeValue.u.object.values[iv1].name
                        var nodeValuev1 = nodeValue.u.object.values[iv1].value
                        var parsedv1 = false
                        if C.strcmp(nodeNamev1, "headerFrequency") == 0 then
                            if nodeValuev1.type ~= [uint32](3) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                            config.io.headerFrequency = [int32](nodeValuev1.u.integer)
                            parsedv1 = true
                        end
                        if C.strcmp(nodeNamev1, "consoleFrequency") == 0 then
                            if nodeValuev1.type ~= [uint32](3) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                            config.io.consoleFrequency = [int32](nodeValuev1.u.integer)
                            parsedv1 = true
                        end
                        if C.strcmp(nodeNamev1, "restartEveryTimeSteps") == 0 then
                            if nodeValuev1.type ~= [uint32](3) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                            config.io.restartEveryTimeSteps = [int32](nodeValuev1.u.integer)
                            parsedv1 = true
                        end
                        if C.strcmp(nodeNamev1, "wrtRestart") == 0 then
                            if nodeValuev1.type ~= [uint32](5) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                            var found = false
                            if C.strcmp(nodeValuev1.u.string.ptr, "OFF") == 0 then
                                config.io.wrtRestart = 1 - 1
                                found = true
                            end
                            if C.strcmp(nodeValuev1.u.string.ptr, "ON") == 0 then
                ...
                if totalParsedv1 < 4 then
                    C.printf("%s\n", "Missing options from config file")
                    C.exit(1)
                end
                parsed = true
            end
            if C.strcmp(nodeName, "Integrator") == 0 then
                var totalParsedv1 = 0
                if nodeValue.type ~= [uint32](1) then
                    C.printf("%s for option %s\n", "Wrong type", nodeName)
                    C.exit(1)
                    ...
                    for iv1 : uint32 = [uint32](0),nodeValue.u.object.length do
                        ...
                        var nodeValuev1 = nodeValue.u.object.values[iv1].value
                        var parsedv1 = false
                        if C.strcmp(nodeNamev1, "finalTime") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "restartIter") == 0 then
                            if nodeValuev1.type ~= [uint32](3) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                            config.integrator.restartIter = [int32](nodeValuev1.u.integer)
                            parsedv1 = true
                        end
                        if C.strcmp(nodeNamev1, "fixedDeltaTime") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "cfl") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "maxIter") == 0 then
                            if nodeValuev1.type ~= [uint32](3) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                            config.integrator.maxIter = [int32](nodeValuev1.u.integer)
                            parsedv1 = true
                        end
                        if parsedv1 then
                            totalParsedv1 = totalParsedv1 + 1
                ...
                if totalParsedv1 < 5 then
                    C.printf("%s\n", "Missing options from config file")
                    C.exit(1)
                end
                parsed = true
            end
            if C.strcmp(nodeName, "Flow") == 0 then
                var totalParsedv1 = 0
                if nodeValue.type ~= [uint32](1) then
                    C.printf("%s for option %s\n", "Wrong type", nodeName)
                    C.exit(1)
                    ...
                    for iv1 : uint32 = [uint32](0),nodeValue.u.object.length do
                        ...
                        var nodeValuev1 = nodeValue.u.object.values[iv1].value
                        var parsedv1 = false
                        if C.strcmp(nodeNamev1, "initCase") == 0 then
                            if nodeValuev1.type ~= [uint32](5) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                            var found = false
                            if C.strcmp(nodeValuev1.u.string.ptr, "Uniform") == 0 then
                                config.flow.initCase = 1 - 1
                                found = true
                            end
                            if C.strcmp(nodeValuev1.u.string.ptr, "Restart") == 0 then
                                config.flow.initCase = 2 - 1
                                found = true
                            end
                            if C.strcmp(nodeValuev1.u.string.ptr, "Perturbed") == 0 then
                                config.flow.initCase = 3 - 1
                                found = true
                            end
                            if C.strcmp(nodeValuev1.u.string.ptr, "TaylorGreen2DVortex") == 0 then
                                config.flow.initCase = 4 - 1
                                found = true
                            end
                            if C.strcmp(nodeValuev1.u.string.ptr, "TaylorGreen3DVortex") == 0 then
                        ...
                        if C.strcmp(nodeNamev1, "powerlawViscRef") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "sutherlandTempRef") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "turbForcing") == 0 then
                            if nodeValuev1.type ~= [uint32](5) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "prandtl") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "sutherlandSRef") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "initParams") == 0 then
                            if nodeValuev1.type ~= [uint32](2) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                            if nodeValuev1.u.array.length ~= [uint32](5) then
                                C.printf("%s for option %s\n", "Wrong length", nodeNamev1)
                                C.exit(1)
                            end
                                for iv2 : int32 = 0,5 do
                                    var rval_i = nodeValuev1.u.array.values[iv2]
                                    if rval_i.type ~= [uint32](4) then
                        ...
                        if C.strcmp(nodeNamev1, "bodyForce") == 0 then
                            if nodeValuev1.type ~= [uint32](2) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                            if nodeValuev1.u.array.length ~= [uint32](3) then
                                C.printf("%s for option %s\n", "Wrong length", nodeNamev1)
                                C.exit(1)
                            end
                                for iv2 : int32 = 0,3 do
                                    var rval_i = nodeValuev1.u.array.values[iv2]
                                    if rval_i.type ~= [uint32](4) then
                                        C.printf("%s for option %s\n", "Wrong element type", nodeNamev1)
                                        C.exit(1)
                        ...
                        if C.strcmp(nodeNamev1, "sutherlandViscRef") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "viscosityModel") == 0 then
                            if nodeValuev1.type ~= [uint32](5) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                            var found = false
                            if C.strcmp(nodeValuev1.u.string.ptr, "Constant") == 0 then
                                config.flow.viscosityModel = 1 - 1
                                found = true
                            end
                            if C.strcmp(nodeValuev1.u.string.ptr, "PowerLaw") == 0 then
                                config.flow.viscosityModel = 2 - 1
                                found = true
                            end
                            if C.strcmp(nodeValuev1.u.string.ptr, "Sutherland") == 0 then
                        ...
                        if C.strcmp(nodeNamev1, "powerlawTempRef") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "constantVisc") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "gasConstant") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "gamma") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                ...
                if totalParsedv1 < 14 then
                    C.printf("%s\n", "Missing options from config file")
                    C.exit(1)
                end
                parsed = true
            end
            if C.strcmp(nodeName, "Particles") == 0 then
                var totalParsedv1 = 0
                if nodeValue.type ~= [uint32](1) then
                    C.printf("%s for option %s\n", "Wrong type", nodeName)
                    C.exit(1)
                    ...
                    for iv1 : uint32 = [uint32](0),nodeValue.u.object.length do
                        ...
                        var nodeValuev1 = nodeValue.u.object.values[iv1].value
                        var parsedv1 = false
                        if C.strcmp(nodeNamev1, "convectiveCoeff") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "diameterMean") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "absorptivity") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "restitutionCoeff") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "density") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "maxNum") == 0 then
                            if nodeValuev1.type ~= [uint32](3) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                            config.particles.maxNum = [int32](nodeValuev1.u.integer)
                            parsedv1 = true
                        end
                        if C.strcmp(nodeNamev1, "bodyForce") == 0 then
                            if nodeValuev1.type ~= [uint32](2) then
                        ...
                        if C.strcmp(nodeNamev1, "maxXferNum") == 0 then
                            if nodeValuev1.type ~= [uint32](3) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                            config.particles.maxXferNum = [int32](nodeValuev1.u.integer)
                            parsedv1 = true
                        end
                        if C.strcmp(nodeNamev1, "maxSkew") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "initNum") == 0 then
                            if nodeValuev1.type ~= [uint32](3) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                            config.particles.initNum = [int32](nodeValuev1.u.integer)
                            parsedv1 = true
                        end
                        if C.strcmp(nodeNamev1, "initCase") == 0 then
                            if nodeValuev1.type ~= [uint32](5) then
                            ...
                            var found = false
                            if C.strcmp(nodeValuev1.u.string.ptr, "Random") == 0 then
                        ...
                        if C.strcmp(nodeNamev1, "initTemperature") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "heatCapacity") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                ...
                if totalParsedv1 < 13 then
                    C.printf("%s\n", "Missing options from config file")
                    C.exit(1)
                end
                parsed = true
            end
            if C.strcmp(nodeName, "BC") == 0 then
                var totalParsedv1 = 0
                if nodeValue.type ~= [uint32](1) then
                    C.printf("%s for option %s\n", "Wrong type", nodeName)
                    C.exit(1)
                    ...
                    for iv1 : uint32 = [uint32](0),nodeValue.u.object.length do
                        ...
                        var nodeValuev1 = nodeValue.u.object.values[iv1].value
                        var parsedv1 = false
                        if C.strcmp(nodeNamev1, "xBCLeftVel") == 0 then
                            if nodeValuev1.type ~= [uint32](2) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "yBCLeftVel") == 0 then
                            if nodeValuev1.type ~= [uint32](2) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "xBCRightVel") == 0 then
                            if nodeValuev1.type ~= [uint32](2) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "xBCRight") == 0 then
                            if nodeValuev1.type ~= [uint32](5) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                            var found = false
                            if C.strcmp(nodeValuev1.u.string.ptr, "Periodic") == 0 then
                                config.bc.xBCRight = 1 - 1
                                found = true
                            end
                            if C.strcmp(nodeValuev1.u.string.ptr, "Symmetry") == 0 then
                                config.bc.xBCRight = 2 - 1
                                found = true
                            end
                            if C.strcmp(nodeValuev1.u.string.ptr, "AdiabaticWall") == 0 then
                                config.bc.xBCRight = 3 - 1
                                found = true
                            end
                            if C.strcmp(nodeValuev1.u.string.ptr, "IsothermalWall") == 0 then
                        ...
                        if C.strcmp(nodeNamev1, "xBCRightTemp") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "zBCRight") == 0 then
                            if nodeValuev1.type ~= [uint32](5) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "yBCRight") == 0 then
                            if nodeValuev1.type ~= [uint32](5) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "zBCLeftTemp") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "xBCLeft") == 0 then
                            if nodeValuev1.type ~= [uint32](5) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "xBCLeftTemp") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "zBCRightTemp") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "yBCLeft") == 0 then
                            if nodeValuev1.type ~= [uint32](5) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "yBCRightVel") == 0 then
                            if nodeValuev1.type ~= [uint32](2) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "zBCRightVel") == 0 then
                            if nodeValuev1.type ~= [uint32](2) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "yBCLeftTemp") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "yBCRightTemp") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "zBCLeftVel") == 0 then
                            if nodeValuev1.type ~= [uint32](2) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "zBCLeft") == 0 then
                            if nodeValuev1.type ~= [uint32](5) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                ...
                if totalParsedv1 < 18 then
                    C.printf("%s\n", "Missing options from config file")
                    C.exit(1)
                end
                parsed = true
            end
            if C.strcmp(nodeName, "Grid") == 0 then
                var totalParsedv1 = 0
                if nodeValue.type ~= [uint32](1) then
                    C.printf("%s for option %s\n", "Wrong type", nodeName)
                    C.exit(1)
                    ...
                    for iv1 : uint32 = [uint32](0),nodeValue.u.object.length do
                        ...
                        var nodeValuev1 = nodeValue.u.object.values[iv1].value
                        var parsedv1 = false
                        if C.strcmp(nodeNamev1, "xWidth") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "xNum") == 0 then
                            if nodeValuev1.type ~= [uint32](3) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                            config.grid.xNum = [int32](nodeValuev1.u.integer)
                            parsedv1 = true
                        end
                        if C.strcmp(nodeNamev1, "yTiles") == 0 then
                            if nodeValuev1.type ~= [uint32](3) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                            config.grid.yTiles = [int32](nodeValuev1.u.integer)
                            parsedv1 = true
                        end
                        if C.strcmp(nodeNamev1, "zTiles") == 0 then
                            if nodeValuev1.type ~= [uint32](3) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                            config.grid.zTiles = [int32](nodeValuev1.u.integer)
                            parsedv1 = true
                        end
                        if C.strcmp(nodeNamev1, "zWidth") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "origin") == 0 then
                            if nodeValuev1.type ~= [uint32](2) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "yNum") == 0 then
                            if nodeValuev1.type ~= [uint32](3) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                            config.grid.yNum = [int32](nodeValuev1.u.integer)
                            parsedv1 = true
                        end
                        if C.strcmp(nodeNamev1, "yWidth") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "xTiles") == 0 then
                            if nodeValuev1.type ~= [uint32](3) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                            config.grid.xTiles = [int32](nodeValuev1.u.integer)
                            parsedv1 = true
                        end
                        if C.strcmp(nodeNamev1, "zNum") == 0 then
                            if nodeValuev1.type ~= [uint32](3) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                            config.grid.zNum = [int32](nodeValuev1.u.integer)
                            parsedv1 = true
                        end
                        if parsedv1 then
                            totalParsedv1 = totalParsedv1 + 1
                ...
                if totalParsedv1 < 10 then
                    C.printf("%s\n", "Missing options from config file")
                    C.exit(1)
                end
                parsed = true
            end
            if C.strcmp(nodeName, "Radiation") == 0 then
                var totalParsedv1 = 0
                if nodeValue.type ~= [uint32](1) then
                    C.printf("%s for option %s\n", "Wrong type", nodeName)
                    C.exit(1)
                    ...
                    for iv1 : uint32 = [uint32](0),nodeValue.u.object.length do
                        ...
                        var nodeValuev1 = nodeValue.u.object.values[iv1].value
                        var parsedv1 = false
                        if C.strcmp(nodeNamev1, "emissSouth") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "tempSouth") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "qa") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "yNum") == 0 then
                            ...
                            config.radiation.yNum = [int32](nodeValuev1.u.integer)
                            parsedv1 = true
                        end
                        if C.strcmp(nodeNamev1, "tempNorth") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "emissWest") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "emissEast") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "tempWest") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "xNum") == 0 then
                            ...
                            config.radiation.xNum = [int32](nodeValuev1.u.integer)
                            parsedv1 = true
                        end
                        if C.strcmp(nodeNamev1, "emissDown") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "emissUp") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "tempDown") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "zNum") == 0 then
                            ...
                            config.radiation.zNum = [int32](nodeValuev1.u.integer)
                            parsedv1 = true
                        end
                        if C.strcmp(nodeNamev1, "emissNorth") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "tempEast") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "qs") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
                        ...
                        if C.strcmp(nodeNamev1, "tempUp") == 0 then
                            if nodeValuev1.type ~= [uint32](4) then
                                C.printf("%s for option %s\n", "Wrong type", nodeNamev1)
                                C.exit(1)
                            end
...
terra parseInputFile(filename : rawstring)
  var num_configs = 0
  var filenames : &rawstring = [&rawstring](nil)
  ...
  C.fscanf(f, "%d\n", &num_configs)
  filenames = [&rawstring](C.malloc(sizeof(rawstring) * num_configs))
  for i = 0, num_configs do
    var filename : int8[512]
    regentlib.assert(C.fscanf(f, "%s\n", filename) == 1, "ill-formed input file")
    var len = C.strlen(filename)
    filenames[i] = [rawstring](C.malloc(len + 1))
    C.memcpy(filenames[i], [rawstring](filename), len + 1)
  end
  return { num_configs = num_configs, filenames = filenames }
end

local terra factorize(grid : Grid, dop : int)
  var num_factors = 0
  var factors = [&int](C.malloc([sizeof(int)] * dop))
  while dop > 1 do
    var factor = 2
    while factor <= dop do
      if dop % factor == 0 then
  ...
  var sz_remain : int[3], extents : int[3]
  sz_remain[0], sz_remain[1], sz_remain[2] = grid.xNum, grid.yNum, grid.zNum
  for k = 0, 3 do extents[k] = 1 end
  for idx = 0, num_factors do
    ...
    extents[next_max] = extents[next_max] * factors[idx]
...
task main()
  ...
  while i < args.argc do
    ...
    elseif C.strcmp(args.argv[i], "-c") == 0 then
      i += 1
      num_cores = C.atoi(args.argv[i])
    end
    i += 1
  end

  var num_configs = 0
  var filenames : &rawstring
  var r = parseInputFile(filename)
  num_configs, filenames = r.num_configs, r.filenames

  var configs : &Config = [&Config](C.malloc([sizeof(Config)] * num_configs))
  for i = 0, num_configs do
    var filename = filenames[i]
    var len = C.strlen(filename)
    configs[i] = parseConfig(filename)
    C.memcpy([&int8](configs[i].filename), filename, len + 1)
    --C.printf("%s %d %d %d\n",
    --  configs[i].filename, configs[i].grid.xNum, configs[i].grid.yNum, configs[i].grid.zNum)
  end

  var total_volume = 0
  for i = 0, num_configs do
    total_volume += configs[i].grid:grid_volume()
  end

  var remaining_cores = num_cores
  for i = 0, num_configs do
    var assigned_cores =
      configs[i].grid:grid_volume() * num_cores / total_volume
    assigned_cores min= assigned_cores
    remaining_cores -= assigned_cores
    regentlib.assert(assigned_cores > 0, "static scheduling failed")
    regentlib.assert(remaining_cores >= 0, "static scheduling failed")
    var extents = factorize(configs[i].grid, assigned_cores)
    configs[i].grid.xTiles = extents[0]
    configs[i].grid.yTiles = extents[1]
    configs[i].grid.zTiles = extents[2]
    C.printf("configuration %d: %d x %d x %d / %d x %d x %d\n", i,
...
regentlib.saveobj(main, "soleil_tgv_algebraic", "executable", cmapper.register_mappers,
                  { "-lm", "-ljsonparser", "-L" .. json_lib_dir,
                           "-lensemble_mapper", "-L" .. root_dir })
