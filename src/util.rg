...
function Exports.mkPartitionIsInterior(r_istype, fs)
  local partitionIsInterior
  if r_istype == int3d then
    __demand(__inline)
    task partitionIsInterior(r : region(ispace(int3d), fs), halo : int3d)
      ...
      regentlib.c.legion_multi_domain_point_coloring_color_domain(coloring, int1d(1), rect3d{
        lo = int3d{ halo.x,        halo.y,        halo.z        },
        hi = int3d{ halo.x+Nx-1,   halo.y+Ny-1,   halo.z+Nz-1   }})
      regentlib.c.legion_multi_domain_point_coloring_color_domain(coloring, int1d(0), rect3d{
        lo = int3d{ 0,             0,             0             },
      ...
      var p = partition(disjoint, r, coloring, ispace(int1d,2))
      regentlib.c.legion_multi_domain_point_coloring_destroy(coloring)
      return p
    end
  elseif r_istype == int2d then
    __demand(__inline)
    task partitionIsInterior(r : region(ispace(int2d), fs), halo : int2d)
      var Nx = r.bounds.hi.x - 2*halo.x + 1
      var Ny = r.bounds.hi.y - 2*halo.y + 1
      regentlib.assert(r.bounds.lo == int2d{0,0}, "Can only partition root region")
      var coloring = regentlib.c.legion_multi_domain_point_coloring_create()
      regentlib.c.legion_multi_domain_point_coloring_color_domain(coloring, int1d(1), rect2d{
        lo = int2d{ halo.x,        halo.y,       },
        hi = int2d{ halo.x+Nx-1,   halo.y+Ny-1   }})
      regentlib.c.legion_multi_domain_point_coloring_color_domain(coloring, int1d(0), rect2d{
        lo = int2d{ 0,             0,            },
  ...
  elseif r_istype == int1d then
    __demand(__inline)
    task partitionIsInterior(r : region(ispace(int1d), fs), halo : int64)
      var N = int64(r.bounds.hi - 2*halo + 1)
      regentlib.assert(r.bounds.lo == 0, "Can only partition root region")
      var coloring = regentlib.c.legion_multi_domain_point_coloring_create()
      regentlib.c.legion_multi_domain_point_coloring_color_domain
      (coloring, int1d(0), rect1d{ lo = 0,      hi = halo-1      })
      regentlib.c.legion_multi_domain_point_coloring_color_domain
      (coloring, int1d(1), rect1d{ lo = halo,   hi = halo+N-1    })
      regentlib.c.legion_multi_domain_point_coloring_color_domain
      (coloring, int1d(0), rect1d{ lo = halo+N, hi = r.bounds.hi })
      var p = partition(disjoint, r, coloring, ispace(int1d,2))
      regentlib.c.legion_multi_domain_point_coloring_destroy(coloring)
      return p
    end
